...
const texturesPaths = ['./src/SFS/Example/Color Textures', './src/SFS/Example/Shape Textures'];
...
const main = () => {
    ...
    const shape_contents = fs.readdirSync(texturesPaths[1], {withFileTypes: true});
    
    col_contents.forEach((value: fs.Dirent) => {
        const name = value.name;

        if (!value.isFile || !acceptableExtensions.includes(getFileExtension(name))) return;
